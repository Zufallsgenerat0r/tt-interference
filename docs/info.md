...
A single virtual "pointer" slowly traces a spiral from screen centre
outward and back. Source A sits at the pointer, source B at its
point-mirror `(640-x, 480-y)` — directly analogous to the JS original
where the second source is the mirror of the mouse position.
...
latch, and the counters needed for spiral phase. Dots don't carry
inertia across frames; the JS version's per-dot damped-spring
smoothing is the one visible omission.
...
black, warping around the two source centres as the spiral breathes
outward and back.

Use the input DIP switches to adjust parameters:

- `ui_in[1:0]`: palette (00=white, 01=cyan, 10=magenta, 11=yellow)
- `ui_in[2]`: freeze the spiral pointer at its current position
- `ui_in[3]`: halve the rotation speed
